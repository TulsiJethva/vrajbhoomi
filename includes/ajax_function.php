...
if(isset($_REQUEST['datewisepanelshow'])){
  global $db;    
    ...
    $str_to_convert_date = strtotime($fromdate);
...
if(isset($_REQUEST['datewisepanelshowBK'])){
  global $db;    
    $movieid = isset($_POST['movieid'])?trim($_POST['movieid']):'';
    $movie_qry = "select * from suraj_movie where suraj_movie_id_pk ='".$movieid."'";
    $movieinfomation = $db->selectOne($movie_qry);
...
//     if ($res2 != false) {
//       foreach ($res2 as $res_state) {
//         $stime= $res_state['movie_assign_start_time'];
//         $new_time= strtotime($stime) + 3600;
//         $onehouradvance = date('g:i A',$new_time);
//         if(strtotime($onehouradvance)>=strtotime($time)){
//             $strBox .='<option value="'.$res_state['movie_assign_start_time'].'-'.$res_state['movie_assign_end_time'].'">' . $res_state['movie_assign_start_time'].' - '.$res_state['movie_assign_end_time'].'</option>';
//           }
//       }
//     }
...
if(isset($_REQUEST['direct_print'])){
  ...
  $sql_assign = 'SELECT 
    ...
    if(!empty($seat_selection)){
        ...
        $returnarray['ticketbookid']=$finalprint_id['ticket_book_id'];
...
if(isset($_REQUEST['PrintStatusUpdate'])){
    global $db;
    $ary=ltrim($_POST['ticketbookid'], '-');
    $exploadary=explode('-',$ary);
    $ticketbookidexpload=implode(',',$exploadary);
    $q="update suraj_movie_ticket_book set is_print='1' WHERE book_id IN (".$ticketbookidexpload.")";
    if($db->query($q)){
...
if(isset($_REQUEST['Pending_Ticket_Print'])){
    $bookingid=$_POST['bookingid'];
    
    global $db;
    
    $sql_book = 'SELECT book_id FROM suraj_movie_ticket_book WHERE is_delete="0" and is_print="0"';
    $bookdetails = $db->selectOne($sql_book);
    if(!empty($bookdetails)){
        $finalprint_id=array();
        $finalprint_id['ticket_book_id']='';
        $finalprint_id['ticket_book_id'] .= "-".$bookingid;
        $returnarray = array();
        if($finalprint_id['ticket_book_id']!=''){ 
          $getfileurl =TICKET_SITE.'get_ticket_print?suraj_ticket_print='.$finalprint_id['ticket_book_id'];
          //$getticktprint = file_get_contents($getfileurl);
